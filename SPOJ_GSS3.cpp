// monera
// only change from gss1 is modify function on single element
#include <bits/stdc++.h>
...
void build(int id,int l,int r)
{
	...
	tree[id].mv=max(tree[id*2].mv,max(tree[id*2+1].mv,tree[id*2].right+tree[id*2+1].left));
	
}

void modify(int x,int p,int id,int l,int r)
{
	if(r-l<2)
	{
		ar[p]=x;
		tree[id].ts=tree[id].mv=tree[id].left=tree[id].right=ar[p];
		return ;
	}
	...
	if(p<mid)
	modify(x,p,id*2,l,mid);
	
	else
	modify(x,p,id*2+1,mid,r);
	
	tree[id].ts=tree[id*2].ts + tree[id*2+1].ts;
	tree[id].left=max(tree[id*2].left,tree[id*2].ts+tree[id*2+1].left);
	tree[id].right=max(tree[id*2+1].right,tree[id*2+1].ts+tree[id*2].right);
	tree[id].mv=max(tree[id*2].mv,max(tree[id*2].right+tree[id*2+1].left,tree[id*2+1].mv));
